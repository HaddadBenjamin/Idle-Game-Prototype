...
public class EventManagerParamsIntAndInt<EnumerationType> where EnumerationType : struct, IConvertible
{
    public delegate void Delegate(int paramA, int paramB);
    ...
    #region Constructor
    public EventManagerParamsIntAndInt()
    {
    ...
    /// </summary>
    /// <param name="enumeration"></param>
    public void CallEvent(EnumerationType enumeration, int paramA, int paramB)
    {
        Delegate action = this.GetEvent(enumeration);

        if (null != action)
            action(paramA, paramB);
    }
    #endregion
}